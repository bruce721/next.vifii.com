                            ...
                            GObjectConstructParam  *params)
{
  ...
  if (!error)
  {
    ...
    if (cache && cache[0] != '\0')
      ...
      uri = hildon_desktop_background_get_filename (background);
                           ...
                           HDHomeWindow                *window)
{
  ...
  if (error)
    {
      ...
      if (dbus_g_error_has_name (error,
        ...
        text = HH_NO_CONNECTION_TEXT;
                                       ...
                                       "org.freedesktop.DBus.GLib.UnmappedError.BackgroundManagerErrorQuark.Code3")))
  ...
  if (!priv->kill_greeting_idle)
    priv->kill_greeting_idle = g_idle_add ((GSourceFunc)kill_greeting, NULL);
}
  ...
  if (HILDON_IS_HOME_AREA (area))
    {
      ...
      {
        /* If there is no local file, we use the system-wide one
         * and copy it locally */
        gchar *content;
        gsize length;
        ...
        if (g_file_get_contents (filename,
                                 ...
                                 NULL))
        {
          g_file_set_contents (user_filename,
                               content,
                               length,
                               NULL);
        }
                           ...
                           gboolean              is_off)
{
  ...
  if (!priv->is_background && HILDON_IS_HOME_AREA (area))
    {
      gtk_container_foreach (GTK_CONTAINER (area),
                             (GtkCallback)hildon_desktop_home_item_set_is_background,
                             GINT_TO_POINTER (is_off));
    }
