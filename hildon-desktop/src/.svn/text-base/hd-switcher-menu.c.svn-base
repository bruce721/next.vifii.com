...
#define AS_MENU_BUTTON_HIGHLIGHTED_NAME "hildon-navigator-small-button4-highlight"
...
struct _HDSwitcherMenuPrivate
{
  ...
  GtkWidget		   *icon;
  GdkPixbuf                *animation_icon;
  
  GtkTreeIter		   *last_iter_added;

  guint                     blinking_notification;
...
static void hd_switcher_menu_replace_blinking_icon (HDSwitcherMenu *switcher, GdkPixbuf *icon);
...
static void hd_switcher_menu_reset_main_icon (HDSwitcherMenu *switcher, gboolean destroy_animation);
...
hd_switcher_menu_init (HDSwitcherMenu *switcher)
{
  ...
  switcher->priv->blinking_notification = -1;
  
  switcher->priv->system_inactivity = FALSE;

  switcher->priv->animation_icon = NULL;
...
static gboolean  
hd_switcher_menu_eventbox_keypress_cb (GtkWidget      *widget,
			               GdkEventKey    *event,
			               HDSwitcherMenu *switcher)
{
			      ...
			      HDSwitcherMenu *switcher)
{
  ...
  hd_switcher_menu_changed_stack_cb (switcher->hdwm, 
		                     HD_WM_ENTRY_INFO (hd_wm_get_active_window ()),
				     switcher); 
...
#ifdef HAVE_LIBOSSO
static void
hd_switcher_menu_refresh_notification_items (HDSwitcherMenu *switcher)
{
  GList *children = NULL, *l;
...
hd_switcher_menu_display_state_cb (osso_display_state_t state, gpointer data)
{
  ...
  if (system_inactivity != priv->system_inactivity)
  {
    ...
    if (!priv->system_inactivity && GDK_IS_PIXBUF (priv->animation_icon))
      hd_switcher_menu_replace_blinking_icon (switcher, 
		                              gdk_pixbuf_copy (priv->animation_icon));
    else
      hd_switcher_menu_reset_main_icon (switcher, FALSE);
  }
}
			      ...
			      GObjectConstructParam *params)
{
  ...
  GdkPixbuf *pixbuf;
  GtkWidget *button, *arrow, *event_box;
  ...
  gtk_widget_set_name (event_box, 
		       HD_SWITCHER_MENU_APP_MENU_NAME);

  g_signal_connect (G_OBJECT (event_box),
		    "key-press-event",
		    G_CALLBACK (hd_switcher_menu_eventbox_keypress_cb),
		    switcher);
  ...
  gtk_widget_show (GTK_WIDGET (switcher->priv->home_item));
...
#ifdef HAVE_LIBOSSO
  hd_switcher_menu_osso_init (switcher);
#endif
...
hd_switcher_menu_finalize (GObject *object)
{
  ...
  if (switcher->priv->animation_icon != NULL)
  {
    g_object_unref (switcher->priv->animation_icon);
    switcher->priv->animation_icon = NULL;
  }
...
hd_switcher_menu_create_clear_notifications_button (HDSwitcherMenu *switcher)
{
  ...
  gtk_widget_show_all (switcher->priv->clear_events_menu_item);
...
hd_switcher_menu_replace_blinking_icon (HDSwitcherMenu *switcher, GdkPixbuf *icon)
{
  ...
  switcher->priv->animation_icon = g_object_ref (icon);
    
  if (!switcher->priv->is_open && !switcher->priv->system_inactivity)
  {
...
hd_switcher_menu_update_highlighting (HDSwitcherMenu *switcher, gboolean state)
{
  if (state)
  {
    gtk_widget_set_name (GTK_WIDGET (switcher), AS_MENU_BUTTON_HIGHLIGHTED_NAME);
    gtk_widget_set_name (switcher->priv->toggle_button, AS_MENU_BUTTON_HIGHLIGHTED_NAME);
  }	  
  else
  {
...
hd_switcher_menu_check_content (HDSwitcherMenu *switcher)
{
  ...
  {    
    ...
    else
    {
       ...
       * menu opens. */
      gtk_widget_set_size_request (switcher->priv->notifications_window,
		                   AS_MENU_PANE_WIDTH, 1);

      gtk_widget_queue_resize (switcher->priv->notifications_window);
...
static void 
hd_switcher_menu_reset_main_icon (HDSwitcherMenu *switcher, gboolean destroy_animation)
{
  if (GTK_BIN (switcher->priv->toggle_button)->child != switcher->priv->icon)
  {
  ...
  if (gtk_tree_model_get_iter_first (nm, &iter))
  {
    ...
    do
    {
	  ...
	  children = g_list_append (children, 
        ...
        hd_switcher_menu_item_set_blinking (HD_SWITCHER_MENU_ITEM (menu_item), 
			                    !ack && !switcher->priv->system_inactivity);
	...
	children = g_list_append (children, menu_item);
	  ...
	  switcher->priv->blinking_notification = id;
          ...
          icon = hd_switcher_menu_get_icon_from_theme (icon_name,
                                                       AS_ICON_SIZE);
          if (icon)
          {
            hd_switcher_menu_replace_blinking_icon (switcher,
...
static void
hd_switcher_menu_refresh_application_items (HDSwitcherMenu *switcher)
{
  GList *children = NULL, *l;
  ...
  for (l = children; l != NULL; l = g_list_next (l))
  {
    if (HD_IS_SWITCHER_MENU_ITEM (l->data))
    {
      ...
      if (hd_wm_entry_info_is_urgent (info) &&
          ...
          !switcher->priv->system_inactivity)
      {
        hd_switcher_menu_item_set_blinking (HD_SWITCHER_MENU_ITEM (l->data), TRUE);
      }
      else if (hd_switcher_menu_item_is_blinking (HD_SWITCHER_MENU_ITEM (l->data)))
      {
        hd_switcher_menu_item_set_blinking (HD_SWITCHER_MENU_ITEM (l->data), FALSE);
      }
    }
				  ...
				  HDSwitcherMenu *switcher)
{
  ...
  /* The rest does not apply to hibernating entries (hopefully) */
      ...
      !hd_wm_entry_info_get_ignore_urgent (info))
  {
    if (menu_item)
    {
      ...
      if (!switcher->priv->system_inactivity)
      {
	...
	switcher->priv->last_urgent_info = info;
	   ...
	   hd_switcher_menu_item_is_blinking (HD_SWITCHER_MENU_ITEM (menu_item)))
  {
...
hd_switcher_menu_show_menu_cb (HDWM *hdwm, HDSwitcherMenu *switcher)
{
  ...
  {
#if HAVE_DECL_GTK_WINDOW_CLOSE_OTHER_TEMPORARIES
                                          ...
                                          HDSwitcherMenu *switcher)
{
  ...
  for (l = children; l != NULL; l = g_list_next (l))
  {
    if (HD_IS_SWITCHER_MENU_ITEM (l->data))	  
    {
      ...
      if (menu_item)
      {
        if (switcher->priv->blinking_notification == id)
        {
	  hd_switcher_menu_reset_main_icon (switcher, TRUE);
	  switcher->priv->blinking_notification = -1;
