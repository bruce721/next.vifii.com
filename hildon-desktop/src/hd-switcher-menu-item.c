...
#define AS_TEXT_COLOR              "PluginMenuTextColor"
#define AS_SECONDARY_TEXT_COLOR    "PluginMenuSecondaryTextColor"
			       ...
			       GParamSpec   *pspec)
{
  ...
  switch (prop_id)
  {
    ...
    case MENU_PROP_NOT_ICON:
      ...
      if (priv->notification_icon)
        g_object_ref (priv->notification_icon);
      break;
			           ...
			           GObjectConstructParam *construct_params)
{
  ...
  hildon_helper_set_logical_color (priv->label2,
                                   GTK_RC_FG,
                                   GTK_STATE_NORMAL,
                                   AS_SECONDARY_TEXT_COLOR);
  ...
  gtk_misc_set_alignment (GTK_MISC (priv->label2), 0.0, 0.1);
  ...
  gtk_misc_set_alignment (GTK_MISC (priv->label), 0.0, 0.9);
  		                      ...
  		                      HDWMEntryInfo        *info)
{
  ...
  gchar *window_name = NULL, *app_name = NULL;
  ...
  app_name = hd_wm_entry_info_get_app_name (priv->info);
  if (app_name)
  {
    gtk_label_set_text (GTK_LABEL (priv->label),
                        app_name);
  ...
  window_name = hd_wm_entry_info_get_window_name (priv->info);
  if (window_name)
  {
    gtk_label_set_text (GTK_LABEL (priv->label2),
                        window_name);
